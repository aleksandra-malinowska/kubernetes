...
var _ = framework.KubeDescribe("Cluster size autoscaler scalability [Slow]", func() {
	...
	It("should scale down empty nodes [Feature:ClusterAutoscalerScalability3]", func() {
		...
		framework.ExpectNoError(framework.WaitForClusterSize(f.ClientSet, totalNodes, scaleDownTimeout))
	})
	...
	//	It("shouldn't scale up with unschedulable pods pending [Feature:ClusterAutoscalerScalability6]", func() {
...
func simpleScaleUpTest(f *framework.Framework, config *scaleUpTestConfig) func() error {
	...
	timeTrack(start, "Scale up to %v", config.expectedResult.nodes))
	return func() error {
